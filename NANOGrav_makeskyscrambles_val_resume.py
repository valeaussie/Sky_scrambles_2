...
def get_scrambles(orf_true, Omega, f, Pij, N=10, thresh=0.1, resume=False):
    ...
    if resume == False:
        ...
        print("resume")
        orfs = list(np.load("/fred/oz002/vdimarco/sky_scrambles/skies/results/NANOGrav_makeskyscrambles_val_orfs.npy"))
        rejections = list(pd.read_csv("/fred/oz002/vdimarco/sky_scrambles/skies/results/2_final_NANOGrav_n_tries_many.csv", index_col=None).values.squeeze())
    ...
    while n_accep < N:
        ...
        if reject == False:
            ...
            np.save("/fred/oz002/vdimarco/sky_scrambles/skies/results/NANOGrav_makeskyscrambles_val_orfs", np.array(orfs))

            start = time.time()

            pd.DataFrame(rejections).to_csv('/fred/oz002/vdimarco/sky_scrambles/skies/results/2_final_NANOGrav_n_tries_many.csv', header=None, index=None)
            ...
            plt.savefig("/fred/oz002/vdimarco/sky_scrambles/skies/plots/NANOGrav_nokill.png")
            plt.clf()
