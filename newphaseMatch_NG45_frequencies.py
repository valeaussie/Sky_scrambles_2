...
def get_scrambles(orf_true, Omega, f, Pij,  N=10, thresh=0.1, resume =False):    
    ...
    if resume == False:
        ...
        print("resume")
        phis = list(np.load("/fred/oz002/users/mmiles/PTA_misspec/Sky_scrambles/results/phase_NG45_phis.npy"))
        rejections = list(pd.read_csv("/fred/oz002/users/mmiles/PTA_misspec/Sky_scrambles/results/phase_NG45.csv",index_col=None).values.squeeze())
    ...
    while n_accep < N:
        ...
        if reject == False:
            phis.append(phi2)
            n_accep += 1
            np.save("/fred/oz002/users/mmiles/PTA_misspec/Sky_scrambles/results/phase_NG45_phis",np.array(phis))
            
            print("accepted phase scrambles = {}".format(n_accep))  
            st = time.time()
            pd.DataFrame(rejections).to_csv('/fred/oz002/users/mmiles/PTA_misspec/Sky_scrambles/results/phase_NG45.csv', header=None, index=None) 
            ...
            plt.savefig("/fred/oz002/users/mmiles/PTA_misspec/Sky_scrambles/plots/phase_NG45.png")
            plt.clf()
...
print(n_i)
pd.DataFrame(c).to_csv('/fred/oz002/users/mmiles/PTA_misspec/Sky_scrambles/results/phase_NG45.csv', header=None, index=None) 
