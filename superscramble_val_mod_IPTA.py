...
def get_scrambles(orf_true, Omega, f, Pij,  N=10, thresh=0.1, resume =False):    
    ...
    if resume == False:
        ...
        print("resume")
        orfs = list(np.load("/fred/oz002/users/mmiles/PTA_misspec/Sky_scrambles/results/super_IPTA_2hourkill_orfs.npy"))
        phis = list(np.load("/fred/oz002/users/mmiles/PTA_misspec/Sky_scrambles/results/super_IPTA_2hourkill_phis.npy"))
        rejections = list(pd.read_csv("/fred/oz002/users/mmiles/PTA_misspec/Sky_scrambles/results/super_IPTA_2hourkill.csv",index_col=None).values.squeeze())
    ...
    while n_accep < N:
        ...
        if reject == False:
            ...
            orfs.append(new_orf)
            n_accep += 1
            np.save("/fred/oz002/users/mmiles/PTA_misspec/Sky_scrambles/results/super_IPTA_2hourkill_phis",np.array(phis))
            np.save("/fred/oz002/users/mmiles/PTA_misspec/Sky_scrambles/results/super_IPTA_2hourkill_orfs",np.array(orfs))
            
            print("accepted super scrambles = {}".format(n_accep))  
            st = time.time()
            pd.DataFrame(rejections).to_csv('/fred/oz002/users/mmiles/PTA_misspec/Sky_scrambles/results/super_IPTA_2hourkill.csv', header=None, index=None) 
            ...
            plt.savefig("/fred/oz002/users/mmiles/PTA_misspec/Sky_scrambles/plots/super_IPTA_2hourkill.png")
            plt.clf()
...
print(n_i)
pd.DataFrame(c).to_csv('/fred/oz002/users/mmiles/PTA_misspec/Sky_scrambles/results/super_IPTA_2hourkill.csv', header=None, index=None) 
