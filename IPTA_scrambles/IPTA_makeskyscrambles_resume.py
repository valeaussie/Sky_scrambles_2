...
def get_scrambles(orf_true, Omega, f, Pij, N=10, thresh=0.1, resume=False):
    ...
    if resume == False:
        ...
        print("resume")
        orfs = list(np.load("/fred/oz002/vdimarco/IPTA_scrambles/results/IPTA_makeskyscrambles_orfs.npy"))
        rejections = list(pd.read_csv("/fred/oz002/vdimarco/IPTA-scrambles/results/IPTA_n_tries_many.csv", index_col=None).values.squeeze())
    ...
    while n_accep < N:
        ...
        if reject == False:
            ...
            np.save("/fred/oz002/vdimarco/IPTA_scrambles/results/IPTA_makeskyscrambles_orfs", np.array(orfs))

            start = time.time()

            pd.DataFrame(rejections).to_csv('/fred/oz002/vdimarco/IPTA_scrambles/results/IPTA_n_tries_many.csv', header=None, index=None)
            ...
            plt.savefig("/fred/oz002/vdimarco/IPTA_scrambles/plots/IPTA_nokill.png")
            plt.clf()
