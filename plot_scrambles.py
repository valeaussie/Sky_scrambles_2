...
cum_accepted = np.cumsum(n_tries)
cum_num_of_tries = np.arange(0, len(n_tries))
...
#-- new code
fig, ax = plt.subplots()
ax.step(cum_num_of_tries, cum_accepted)
xticks = np.linspace(0, 40000, 9)
xticklabels = ['{:.1f}'.format(i/1000) for i in xticks]
...
plt.savefig('/fred/oz002/vdimarco/sky_scrambles/skies/plots/test_plot.pdf', bbox_inches='tight' )
