...
with open("NANOGrav_S_I_45psrs.csv", "w") as S_I_file:
    writer = csv.writer(S_I_file)
    for i in range(33):
        writer.writerow(specs[i].S_I)
with open("NANOGrav_frequencies_45psrs.csv", "w") as freqs_file:
    writer = csv.writer(freqs_file)
    writer.writerow(specs[0].freqs)
sc = hsen.DeterSensitivityCurve(specs)
with open("NANOGrav_Omega_45psrs.csv", "w") as Omega_file:
