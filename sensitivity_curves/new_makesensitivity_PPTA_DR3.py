...
#import par and tim files and noise dictionary for DR2 PPTA
par_tim_dir = '/fred/oz002/vdimarco/sky_scrambles/skies/sensitivity_curves/ppta_dr3/data/'
noise_dir = '/fred/oz002/vdimarco/sky_scrambles/skies/sensitivity_curves/ppta_dr3/noisefiles/'
pars = sorted(glob.glob(par_tim_dir+'*.par'))
tims = sorted(glob.glob(par_tim_dir+'*.tim'))
noise_files = sorted(glob.glob(noise_dir+'*.json'))


#extract name of pulsars
psr_list = noise_files[:]
for i in range(len(noise_files)):
    psr_list[i] = psr_list[i].replace("_singlePsrNoise_sw_nesw0_noise.json", "")
    psr_list[i] = psr_list[i].replace("/fred/oz002/vdimarco/sky_scrambles/sensitivity_curves/ppta_dr3/noisefiles/", "")
...
#print outputs
with open("PPTA_DR3_S_I_alltimes.csv", "w") as S_I_file:
    writer = csv.writer(S_I_file)
    for i in range(len(name)):
        writer.writerow(specs[i].S_I)
with open("PPTA_DR3_frequencies_alltimes.csv", "w") as freqs_file:
    writer = csv.writer(freqs_file)
    writer.writerow(specs[0].freqs)
sc = hsen.DeterSensitivityCurve(specs)
with open("PPTA_DR3_Omega_alltimes.csv", "w") as Omega_file:
